...
This project started as the inversion of a joke. In December 2025 the linguist Adam Aleksic, better known as [@etymologynerd](https://www.tiktok.com/@etymologynerd/video/7584610585300438285), posted a video proposing the eigenslur: embed the vocabulary of derogatory language and the vectors share a direction, so something must sit at its far end, captioned "the eigenslur that can be spoken is not the true eigenslur." The thought experiment lives in longer form at [eigenslur.org](https://eigenslur.org/), which calls it "the principal component of offense" and never ran the computation. A [response video](https://www.tiktok.com/@zach.zeta.s/video/7640238795803151646) by @zach.zeta.s then asked the mirror-image question: could the same construction capture the direction of actions that aim at human flourishing, an eigenvalue of virtue? That video ended with an open invitation to attempt the idea rigorously, which is what this repository does. We also went back and ran the original negative direction; the write-up is in [docs/eigenslur.md](docs/eigenslur.md).
...
- **Provenance of the question:** [@etymologynerd's eigenslur video](https://www.tiktok.com/@etymologynerd/video/7584610585300438285) (Adam Aleksic, December 2025), [eigenslur.org](https://eigenslur.org/) (the thought experiment in long form, never executed), and [@zach.zeta.s's response video](https://www.tiktok.com/@zach.zeta.s/video/7640238795803151646) proposing the flourishing-direction inversion this repo implements. Our run of the original direction is in [docs/eigenslur.md](docs/eigenslur.md).

## 8. The other pole: the eigenslur

The idea that started all of this pointed the other way, and it deserves the fuller telling. In December 2025, [@etymologynerd](https://www.tiktok.com/@etymologynerd/video/7584610585300438285) (Adam Aleksic, the linguist behind the book *Algospeak*) posted a one-minute video observing that when a language model embeds the vocabulary of derogatory language, the embeddings are not scattered at random: they share a direction, and linear algebra guarantees that some point projects furthest along it. He named that point the eigenslur and captioned the video "the eigenslur that can be spoken is not the true eigenslur," a Tao Te Ching joke that turned out to be a real technical prediction, since the extreme point of a tokenizer vocabulary is more likely a meaningless byte fragment than a word. The longer thought experiment at [eigenslur.org](https://eigenslur.org/) calls it "the principal component of offense, the eigenvector of linguistic cruelty" and leaves the computation unrun. A [response video](https://www.tiktok.com/@zach.zeta.s/video/7640238795803151646) by @zach.zeta.s flipped the question toward human flourishing, which became the main study above.

We went back and ran the original direction too, responsibly: a denigration axis fit from 18 respect/contempt pairs containing no slurs and no group-targeted language, since the interesting claim was always geometric rather than lexical.
